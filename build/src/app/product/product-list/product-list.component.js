"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
var core_1 = require("@angular/core");
var ProductListComponent = /** @class */ (function () {
    function ProductListComponent(productService) {
        this.productService = productService;
        this.products = [];
        this.productToDisplay = null;
    }
    // Load all the products when starting the view.
    ProductListComponent.prototype.ngOnInit = function () {
        this.loadAll();
    };
    // If new product created, we add it to the list.
    ProductListComponent.prototype.ngOnChanges = function () {
        if (this.productToDisplay !== null) {
            this.products.push(this.productToDisplay);
        }
    };
    // Delete a product. 
    ProductListComponent.prototype.delete = function (id) {
        var _this = this;
        this.productService.delete(id).then(function (result) { return _this.loadAll(); });
    };
    // Load all products.
    ProductListComponent.prototype.loadAll = function () {
        var _this = this;
        this.productService
            .get()
            .then(function (result) {
            _this.products = result;
        });
    };
    __decorate([
        core_1.Input()
    ], ProductListComponent.prototype, "productToDisplay", void 0);
    ProductListComponent = __decorate([
        core_1.Component({
            selector: 'app-product-list',
            templateUrl: './product-list.component.html',
            styleUrls: ['./product-list.component.css']
        })
    ], ProductListComponent);
    return ProductListComponent;
}());
exports.ProductListComponent = ProductListComponent;
